...
const STATUSES = [
    ...
    },
    {
        ...
        shouldRender: (entity, entityType) =>
            isCernMode && entityType === 'equipment' && entity.userDefinedFields.udfchkbox01 === 'true',
    ...
    },
    {
        ...
        shouldRender: (entity, entityType) =>
            isCernMode && entityType === 'equipment' && entity.userDefinedFields.udfchar04 === 'Radioactive',
...
const StatusRow = (props) => {
    const generateCells = (entity, entityType) => {
        return STATUSES.filter((status) => status.shouldRender(entity, entityType)).map((checkedStatus) => (
            <Tooltip key={checkedStatus.key} title={checkedStatus.tooltip}>
                <div style={{ textAlign: 'center', width: '80px' }}>
                    {checkedStatus.icon}
                    <div style={{ fontSize: '0.75rem' }}>{checkedStatus.description}</div>
                </div>
            </Tooltip>
        ));
    };
