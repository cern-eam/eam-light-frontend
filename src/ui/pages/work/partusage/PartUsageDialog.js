...
function PartUsageDialog(props) {
    ...
    const { isDialogOpen, handleError, workorder } = props;

    useEffect(() => {
        let loadLists = (workorder) => {
            Promise.all([WSWorkorders.getPartUsageStores(), WSWorkorders.getWorkOrderActivities(workorder.number)])
                .then((responses) => {
    ...
    }, [isDialogOpen, handleError, workorder]);
    ...
    let loadBinList = (binCode, partCode) => {
        if (!partCode) return;
        WSWorkorders.getPartUsageBin(partUsage.transactionType, binCode, partCode, partUsage.storeCode)
            .then((response) => {
