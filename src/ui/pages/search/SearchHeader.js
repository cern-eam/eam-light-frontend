            ...
            >
                <div id="searchBoxLabel" className="searchBoxLabelHome">
                    ...
                    <div
                        ...
                        className={
                            this.props.searchBoxUp ? 'searchBoxLabelGreetingSearch' : 'searchBoxLabelGreetingHome'
                        }
                    >
                        <span className="FontLatoBlack Fleft Fs30 DispBlock" style={{ color: '#02a2f2' }}>
                ...
                <div
                    id="searchBoxInput"
                    className={this.props.searchBoxUp ? 'searchBoxInputSearch' : 'searchBoxInputHome'}
                >
                    <EAMBarcodeInput
                        updateProperty={(val) => this.props.fetchDataHandler(val, entityTypes)}
                        top={3}
                        right={-7}
                    >
                        <input
                    ...
                    {showTypes && (
                        <div style={{ height: '30px', display: 'flex', direction: 'row', whiteSpace: 'nowrap' }}>
                            {Object.values(SEARCH_TYPES).map(this.renderTypeCheckbox.bind(this))}
                        </div>
                    )}
                    <label id="searchPlaceHolder">
                        {!this.props.keyword && 'Search for Equipment, Work Orders, Parts, ...'}
                    </label>
                </div>
            </div>
        );
