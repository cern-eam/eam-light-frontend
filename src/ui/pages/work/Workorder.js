...
class Workorder extends Entity {
    ...
    getRegions = () => {
        const { applicationData, handleError, showError, showNotification, userData, workOrderLayout } = this.props;
        ...
        return [
            {
                ...
                maximizable: false,
                render: () => (
                    <WorkorderDetails {...commonProps} applicationData={applicationData} userData={userData} />
                ),
                column: 1,
                order: 1,
            ...
            },
            {
                ...
                order: 8,
                ignore:
                    !getTabAvailability(tabs, TAB_CODES.ACTIVITIES) && !getTabAvailability(tabs, TAB_CODES.BOOK_LABOR),
                initialVisibility:
            ...
            },
            {
                ...
                isVisibleWhenNewEntity: true,
                customVisibility: () =>
                    WorkorderTools.isRegionAvailable('CUSTOM_FIELDS_EQP', commonProps.workOrderLayout),
                maximizable: false,
                render: () => (
                    <CustomFields
                        children={this.children}
            ...
            },
            {
                ...
                render: () => (
                    <WorkorderMultiequipment
                        workorder={workorder.number}
                        setEquipmentMEC={this.setEquipmentMEC.bind(this)}
                    />
                ),
                column: 2,
                order: 13,
    ...
    postCopy = () => {
        ...
        isCernMode &&
            this.updateEntityProperty(
                'typeCode',
                fields.workordertype.defaultValue ? fields.workordertype.defaultValue : 'CD'
            );
    ...
    setStatuses(status, type, newwo) {
        WSWorkorder.getWorkOrderStatusValues(this.props.userData.eamAccount.userGroup, status, type, newwo).then(
            (response) => {
    ...
    setTypes(status, type, newwo, ppmwo) {
        WSWorkorder.getWorkOrderTypeValues(this.props.userData.eamAccount.userGroup).then((response) => {
    ...
    setClosingCodes = (prevState) => {
        ...
        if (
            !equalObjectProps(prevWorkorder, workorder, [
                'problemCode',
                'failureCode',
                'causeCode',
                'classCode',
            ...
            Promise.all([
                ...
                const [problemCodeValues, actionCodeValues, causeCodeValues, failureCodeValues] = responses.map(
