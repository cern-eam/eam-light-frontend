...
const WO_FILTERS = {
    ...
    [WO_FILTER_TYPES.OPEN]: {
        ...
        process: (data) => {
            return data.filter(
                (workOrder) =>
                    workOrder.status && ['T', 'C'].every((statusCode) => !workOrder.status.startsWith(statusCode))
            );
        },
    },
    ...(isCernMode
...
function EquipmentWorkOrders(props) {
    ...
    let [workOrderFilter, setWorkOrderFilter] = useState(
        Object.values(WO_FILTER_TYPES).includes(defaultFilter) ? defaultFilter : WO_FILTER_TYPES.ALL
    );
