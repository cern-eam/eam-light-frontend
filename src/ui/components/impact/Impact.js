...
class Impact extends Component {
    ...
    loadWOInfo = () => {
        this.setState(
            () => ({ loading: true }),
            () => {
                WSImpact.getWorkorderInfo(this.props.workorderNumber).then(
                    (response) => {
    ...
    loadActivitiesInfo = () => {
        ...
        if (data.impactActivityId) {
            Promise.all([
                WSImpact.getWorkorderInfosWithSameActivity(data.impactActivityId),
                WSImpact.getImpactActivity(data.impactActivityId),
            ]).then(
                (response) => {
                ...
                (error) => {
                    const errorCode =
                        error && error.response && error.response.body && error.response.body.errors[0]
    ...
    unlinkActivity = () => {
        this.setState(
            ...
            () => {
                ...
                WSImpact.unlinkWorkorder(this.state.woInfo.workordernumber).then(
                    (response) => {
                        this.setState(
                            () => ({ currentActivity: null, loading: false }),
                            () => {
                                this.loadWOInfo();
                            }
                        );
    ...
    attachToExistingActivity = (activityId) => {
        this.setState({ loading: true, loaded: false }, () => {
            WSImpact.linkWorkorderToImpactActivity(this.state.woInfo.workordernumber, activityId).then(
                (response) => {
                    this.loadWOInfo();
    ...
    addAdditionalWorkorders = () => {
        ...
        WSImpact.linkWorkordersToImpactActivity(this.state.currentActivity.id, workorders).then(
            (response) => {
    ...
    renderCreationMode() {
        ...
        return (
            <div>
                <Grid container spacing={1}>
                    <Grid item sm={12} xs={12}>
                        <EISPanel heading="IMPACT Activity">
                            <Button
                                color="primary"
                                onClick={() => this.setActivityCreationVisibility(true)}
                            ...
                            <Button
                                color="primary"
                                onClick={() => this.setActivityGridVisibility(true)}
                                disabled={this.state.loading}
                            >
                ...
                <Dialog
                    onClose={() => this.setActivityCreationVisibility(false)}
                ...
                <Dialog
                    fullScreen
                    onClose={() => this.setActivityGridVisibility(false)}
                ...
                >
                    <BlockUi tag="div" blocking={this.state.loading} style={{ height: '100%', width: '100%' }}>
                        ...
                        <ImpactActGrid
                            attachToExistingActivity={this.attachToExistingActivity}
    ...
    renderError() {
        return <div style={{ padding: 10, backgroundColor: 'pink', color: 'red' }}>{this.state.errorMessage}</div>;
    }
    ...
    renderA() {
        ...
        return (
            <div>
                ...
                <Dialog
                    onClose={() => this.setDetachActivityConfirmationDialogVisible(false)}
                ...
                <Dialog
                    fullScreen
                    onClose={() => this.setAdditionalWorkorderVisibility(false)}
        ...
        return (
            <div style={{ margin: 5, padding: 20, height: '100%', overflowY: 'scroll' }}>
                <BlockUi tag="div" blocking={this.state.loading}>
                    ...
                    {this.state.loaded &&
                        !this.state.woClosedWithoutActivity &&
                        !this.state.error &&
