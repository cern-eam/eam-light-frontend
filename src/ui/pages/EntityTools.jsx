...
import { TABS } from "../components/entityregions/TabCodeMapping";
...
const getSystemParam = (paramName, tabId, screenCode) => {
  if (paramName === "CURRENT_TAB_NAME") {
    return tabId;
  }
  if (paramName === "SYSTEM_FUNCTION_NAME") {
    return screenCode;
  }
} 
...
const getCustomTabUrl = (customTab, tabId, screenCode, equipment, userData, layoutPropertiesMap) => {
  ...
  .reduce((acc, { paramName, paramValue, system, useFieldValue }) => {
    if (paramName) {
      const systemValue = getSystemParam(paramName, tabId, screenCode);
      if (systemValue) {
...
export const getCustomTabRegions = (
  ...
  const alreadyPresentTabs = TABS[screenCode];
  return Object.entries(customTabs).filter(([tabId]) => !alreadyPresentTabs?.includes(tabId)).map(([tabId, tab], index) => {
