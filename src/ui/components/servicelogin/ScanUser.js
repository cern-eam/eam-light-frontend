...
const ScanUser = ({ updateScannedUser, showNotification, handleError }) => {
    ...
    return (
        ...
        >
            ...
            >
                <span className="FontLatoBlack Fleft Fs30 DispBlock" style={{ fontSize: '18px', color: '#02a2f2' }}>
                    {mode === MODES.PERSON ? 'Scan your CERN badge: ' : 'Insert your ID: '}
                </span>
                <Input
                    //ref={that => that.focus()}
                    autoFocus
                    ...
                    onChange={(event) => {
                        ...
                        if (scannerOnly && value && value.length - (cernId || '').length > 1) {
                            ...
                            updateCernId(
                                value && (mode === MODES.PERSON ? value.replace(/\D/g, '') : value.toUpperCase())
                            );
                        }
                    }}
                    placeholder={mode === MODES.PERSON ? 'Person ID' : 'CERN ID, Person ID or Login'}
                    style={{ width: '200px' }}
                    onBlur={(evt) => !scannerOnly && updateUser(evt)}
                    onKeyDown={(event) => {
                        if (scannerOnly) {
