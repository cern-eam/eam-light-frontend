        ...
        return WS.getUserData(currentScreen, screenCode)
            .then((response) => {
                ...
                Promise.all(createPromiseArray(userdata)).then((values) => {
                    ...
                    try {
                        serviceAccounts =
                            values[0].body.data.EL_SERVI && Object.keys(JSON.parse(values[0].body.data.EL_SERVI));
                    } catch (err) {
                        serviceAccounts = [];
                    }
                    dispatch(
...
export function updateScreenLayout(entity, entityDesc, systemFunction, userFunction, tabs) {
    return (dispatch, getState) => {
        ...
        WS.getScreenLayout(userData.eamAccount.userGroup, entity, systemFunction, userFunction, tabs).then(
            (response) => {
                dispatch(
                    updateApplication({
...
function createPromiseArray(userdata) {
    ...
    if (userdata.assetScreen) {
        assetScreenPromise = WS.getScreenLayout(
    ...
    if (userdata.positionScreen) {
        positionScreenPromise = WS.getScreenLayout(
            userdata.eamAccount.userGroup,
            'OBJ',
            'OSOBJP',
    ...
    if (userdata.systemScreen) {
        systemScreenPromise = WS.getScreenLayout(
            userdata.eamAccount.userGroup,
            'OBJ',
            'OSOBJS',
    ...
    if (userdata.partScreen) {
        partScreenPromise = WS.getScreenLayout(
            userdata.eamAccount.userGroup,
            'PART',
            'SSPART',
            userdata.partScreen,
    ...
    if (userdata.workOrderScreen) {
        woScreenPromise = WS.getScreenLayout(
            userdata.eamAccount.userGroup,
            'EVNT',
            'WSJOBS',
    ...
    if (userdata.locationScreen) {
        locationScreenPromise = WS.getScreenLayout(
            userdata.eamAccount.userGroup,
            'LOC',
            'OSOBJL',
