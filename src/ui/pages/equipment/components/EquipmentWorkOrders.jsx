...
import EISTable, {
  TRANSFORM_KEYS,
} from "eam-components/dist/ui/components/table";
    ...
    process: (data) => {
      return data.filter(
        (workOrder) =>
          workOrder.status &&
          ["T", "C"].every(
            (statusCode) => !workOrder.status.startsWith(statusCode)
          )
      );
    },
  },
        ...
        [WO_FILTER_TYPES.MTF]: {
          ...
          process: (data) => {
            return data.filter((workOrder) => {
              return (
                workOrder.mrc &&
                (workOrder.mrc.startsWith("ICF") ||
                  workOrder.mrc.startsWith("MTF"))
              );
            });
          },
        },
...
function EquipmentWorkOrders(props) {
  ...
  const [workOrderFilter, setWorkOrderFilter] = useLocalStorage(
      ...
      .then((responses) => {
        const getDateLabel = (element) => {
          ...
          const elementIsClosed =
            element.status.startsWith("T") || element.status.startsWith("C");
    ...
    <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      {workOrderFilter === WO_FILTER_TYPES.MTF ? (
        ...
        <BlockUi blocking={loadingData} style={{ overflowX: "auto" }}>
          <EISTable
            data={getFilteredWorkOrderList(
              workOrderFilter === WO_FILTER_TYPES.THIS ? workorders : events
            )}
            headers={headers}
            propCodes={propCodes}
            linksMap={linksMap}
