  ...
  const customRenders = {
    part: (
      <CellComponent style={customCellStyle}>
        <Link to={{ pathname: `/part/${row["part"]}` }}>
          {getDisplayValue()}
        </Link>
      </CellComponent>
    ),
    child_equipment: (
      <CellComponent style={customCellStyle}>
        <Link to={{ pathname: `/equipment/${row["child_equipment"]}` }}>
          {getDisplayValue()}
        </Link>
      </CellComponent>
    ),
    description: (
      <CellComponent style={{ minWidth: 150, padding: 4 }}>
        {getDisplayValue()}
      </CellComponent>
    ),
  };
          ...
          extraBodyRender={() => (
            <>
              {!loading && !requestError && !rows.length && (
                <caption>No data to show.</caption>
              )}
              {!loading && requestError && (
                <caption>Failed to load data</caption>
              )}
            </>
          )}
        />
