...
import {
  ...
  DATA_GRID_SORT_DIRECTIONS,
} from "eam-components/dist/ui/components/datagrid/Constants";
...
const customCellRenderer = ({
  ...
  const customRenders = {
    last_repeated_status_color: (
      <CellComponent style={{ backgroundColor: getDisplayValue() }} />
    ),
    mtf_step: (
      <CellComponent>
        <Link to={{ pathname: `/workorder/${row["evt_code"]}` }}>
          {getDisplayValue()}
        </Link>
      </CellComponent>
    ),
    evt_desc: <CellComponent>{getDisplayValue()}</CellComponent>,
  };
  return (
    customRenders[columnMetadata.id] || (
      <CellComponent style={customCellStyle}>{getDisplayValue()}</CellComponent>
    )
  );
};
...
const comparatorsMap = {
  evt_completed: ({ direction, property }) => {
    return (a, b) =>
      (direction === DATA_GRID_SORT_DIRECTIONS.DESC ? 1 : -1) *
      compareAsc(
        parse(a[property], DATE_FORMAT, new Date()),
        parse(b[property], DATE_FORMAT, new Date())
      );
  },
};
...
const getComputedColumnsMetadata = ({ columnsMetadata }) => {
  ...
  return columnsMetadata
    .filter((e) => availableHeaders.includes(e.id))
    .map((c) => ({
      ...c,
    ...
    <EAMTableGridRequestAdapter gridRequest={gridRequest} headers={headers}>
      {({ loading, requestError, rows, columnsMetadata }) => (
        <EAMTable
          ...
          isSortEnabled={(columnMetadata) =>
            !["last_repeated_status_color", "checkbox"].includes(
              columnMetadata.id
            )
          }
          ...
          extraBodyRender={() => (
            <>
              {!loading && !requestError && !rows.length && (
                <caption>No MTF Work Orders to show.</caption>
              )}
              {!loading && requestError && (
                <caption>Failed to load MTF Work Orders</caption>
              )}
            </>
          )}
        />
